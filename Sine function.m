...
%=========================================================================%
%Checking if the number of supplied frequencies and amplitudes are the
%same
if (length(fk) ~= length(Xk))
    error('Frequencies and amplitudes do not match');
    return;
end
%Time axis
tt = linspace(tstart, dur, fs);
xx = zeros(1, length(tt));
%Synthesizing the required signal
for i = 1 : length(fk)
